...
if (banner) {
  ...
  const showBanner = () => {
    ...
    document.cookie = `${cookieName}=1; max-age=${filterCookieExpiration(cookieExpiration)}; path=/`;
  };
  ...
  if (cookieExpiration) {
    const cookieValue = document.cookie.replace(new RegExp("(?:(?:^|.*;\\s*)" + encodeURIComponent(cookieName).replace(/[\-\.\+\*]/g, "\\$&") + "\\s*\\=\\s*([^;]*).*$)|^.*$"), "$1");
    if (cookieValue !== "1") {
      showBanner();
    }
  }
  ...
  const closeButton = banner.querySelector("button.wp-block-block-banner-plugin-close-button");
  if (closeButton) {
